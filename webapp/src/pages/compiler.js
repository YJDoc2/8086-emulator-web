...
ends
...
end start ; set entry point and stop the assembler.`) //State to maintain the code string
	...
	const addAnnotation = (rowNumber, columnNumber, errorText) => {
		...
		setErrorAnnotations(errorAnnotations => [...errorAnnotations, { row: rowNumber, column: columnNumber, type: 'error', text: errorText }])
	}
	...
	const validateAddress = (address) => {
		if(/^[0-9A-F]{5}$/.test(address)) {
			if(address.slice(0, 3) === 'FFF' && (parseInt(address[3]) > 7 || address[3].match(/[A-F]/i))) {
	...
	//Runs once on mount
	useEffect(() => {
		addAnnotation(3, 1, "Syntax Error Long random text is here ok yes test 123") //Annotation goes away when u press enter
			...
			<Grid container spacing={2}>
				<Grid item md={7}>
					<Typography variant="h5" gutterBottom className={classes.flex}>
						...
						<div align="right" className={classes.compileBtnDiv}>
							<Button variant="contained" color="primary" size="small" onClick={compile} disabled={!props.wasm} className={classes.compileButton}> COMPILE </Button>
							<Button variant="outlined" size="small" onClick={runCode} disabled={!compiled||halted} className={compiled?classes.runBtn:classes.topBtn}> Run </Button>
							<Button variant="outlined" size="small" onClick={executeNext} disabled={!compiled||halted} className={compiled?classes.nextBtn:classes.topBtn}> Next </Button>
							<Button variant="outlined" size="small" onClick={stopCode} disabled={!compiled||halted} className={compiled?classes.stopBtn:classes.topBtn}> Stop </Button>
						</div>
					</Typography>
				    {
					    // To Do:
				...
				<Grid item md={5}>
					...
					<Grid container>
						...
						<Grid item xs={5}>
							<Textfield style={{top:-4}} error={!!addressError} size="small" value={startAddress} label="Start Address" onChange={e=>{setStartAddress(e.target.value.toUpperCase());validateAddress(e.target.value.toUpperCase())}} helperText={addressError} className={classes.textF} placeholder="Start Address"/>
						</Grid>
						<Grid item xs={2}>
							<Button variant="outlined" size="large" onClick={saveAddress}> Set </Button>
						</Grid>
					</Grid>
